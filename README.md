...
Tit-for-Tat tracks each object's win/loss streak continuously — that streak
isn't reset when a round or turn ends. An object that was still
mid-retaliation when the previous round ended carries that state into the
next one, so a round doesn't necessarily open with everyone giving.
